			...
			<section id="container">
				<form role="form" method="get">
					<table class="table table-hover">
						...
						<c:forEach items="${supplyList}" var="list">
							<tr>
				...
				<!-- page 이전 1 2 3 4 5 다음 만드세요  -->
					<form action = "supplymange.do" name = "pageForm" method="post">
					...
					<form action="supplymange.do" method="post">
					    <div class="search-wrap" style="display: flex; align-items: center;"> <!-- 인라인 스타일 추가 -->
					        <select class="form-control search-select" name="searchCondition"  > <!-- 인라인 스타일 추가 -->
					            <option value="name" ${pageVO.cri.searchCondition eq 'name' ? 'selected' : '' }>공급사명</option>
					            <option value="phone" ${pageVO.cri.searchCondition eq 'phone' ? 'selected' : '' }>회사번호</option>
					            <option value="id" ${pageVO.cri.searchCondition eq 'id' ? 'selected' : '' }>아이디</option>
					            <option value="addr" ${pageVO.cri.searchCondition eq 'addr' ? 'selected' : '' }>주소</option>
					            <option value="email" ${pageVO.cri.searchCondition eq 'email' ? 'selected' : '' }>이메일</option>
					        </select>
